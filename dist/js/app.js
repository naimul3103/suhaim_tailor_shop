...
function printReceipt() {
  ...
  // Get ORDER NUMBER (not receipt ID) for filename
  let orderNumber = "";

  // First priority: Get from formData (most reliable source)
  if (window.formData && window.formData.orderNumber) {
    orderNumber = window.formData.orderNumber;
  }

  // Second: Try to find order number in receipt content
  // Look for elements that specifically contain order number (not receipt ID)
  if (!orderNumber) {
    // Try to find by looking for "Order" or "ORD-" pattern
    const allTextElements = receiptContent.querySelectorAll("*");
    for (let elem of allTextElements) {
      ...
      // Also check if element contains "Order Number" label
      if (text.includes("Order Number:") || text.includes("Order #:")) {
        const nextElement = elem.nextElementSibling || elem.parentElement;
        if (nextElement) {
          const nextText = nextElement.textContent || "";
          const match = nextText.match(/ORD-\d{4}/);
          if (match) {
            orderNumber = match[0];
            break;
          }
  ...
  // Third: Get from the main form's order number display
  if (!orderNumber) {
    const mainOrderElement = document.getElementById("orderNumber");
    if (mainOrderElement) {
      orderNumber = mainOrderElement.textContent.trim();
  ...
  // Fourth: Try session storage (current order number)
  if (!orderNumber) {
    const sessionOrderNumber = sessionStorage.getItem("currentOrderNumber");
    if (sessionOrderNumber) {
      orderNumber = sessionOrderNumber;
    }
  }

  // Make sure we have ORD- format
  if (orderNumber && !orderNumber.startsWith("ORD-")) {
    // If we somehow got just a number, format it properly
    if (/^\d+$/.test(orderNumber)) {
      orderNumber = `ORD-${orderNumber.padStart(4, "0")}`;
    }
  }
  ...
  console.log("Using order number for filename:", orderNumber); // Debug log

  // Store the original document title
  const originalTitle = document.title;

  // Change the main document title to the order number
...
<body>
  ...
  // Enhanced cleanup - RESTORE ORIGINAL TITLE
  function cleanup() {
  ...
  function executePrint() {
    try {
      // Make sure main document title is still set to order number
      if (document.title !== orderNumber) {
        document.title = orderNumber;
      }
      ...
      // Also handle the main window's afterprint event
